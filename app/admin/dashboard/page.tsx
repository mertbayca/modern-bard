...
export default async function DashboardPage() {
  ...
  const [
    ...
    sql`SELECT SUM(views) as total FROM drafts WHERE published = true`,
    sql`
      SELECT id, title, slug, views, form, published
      FROM drafts
    ...
    <div className="min-h-screen bg-paper dark:bg-ink">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-3 mb-8">
          <div className="lg:col-span-2 bg-white dark:bg-ink-light border border-mist dark:border-ink-light rounded-lg p-6">
                ...
                {popularPosts.map((post, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full bg-sage/10 dark:bg-sage-light/10 text-sage dark:text-sage-light font-bold text-sm">
                        {index + 1}
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-ink/60 dark:text-paper/60">
                          {post.views.toLocaleString()} views • {post.form}
                        </p>
                      </div>
                    </div>
            ...
            {allDrafts.length > 0 ? (
              <div className="space-y-4">
                {allDrafts.slice(0, 5).map((draft) => (
                  <div
                    key={draft.id}
                    className="p-3 border border-mist dark:border-ink-light rounded-md hover:bg-mist/20 dark:hover:bg-ink/20 transition-colors"
                  >
                    <h4 className="font-medium text-sm text-ink dark:text-paper mb-1">
                      {draft.title}
                    </h4>
              ...
              <p className="text-center text-ink/60 dark:text-paper/60 py-8 text-sm">
                No posts yet.
              </p>
            )}
          </div>
